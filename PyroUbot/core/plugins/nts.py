...
async def addnote_cmd(client, message):
    ...
    if reply and note_name:
        ...
        await message.edit("ᴄᴀᴛᴀᴛᴀɴ ʙᴇʀʜᴀsɪʟ ᴅɪ sɪᴍᴘᴀɴ")
    else:
...
async def delnote_cmd(client, message):
    note_name = get_arg(message)
    if not note_name:
        return await message.edit("ᴀᴘᴀ ʏᴀɴɢ ɪɴɢɪɴ ᴀɴᴅᴀ ʜᴀᴘᴜs?")
    note = await get_note(client.me.id, note_name)
    if not note:
        return await message.edit(f"ᴄᴀᴛᴀᴛᴀɴ {note_name} ᴛɪᴅᴀᴋ ᴀᴅᴀ")
    await rm_note(client.me.id, note_name)
    await message.edit(f"ʙᴇʀʜᴀsɪʟ ᴍᴇɴɢʜᴀᴘᴜs ᴄᴀᴛᴀᴛᴀɴ {note_name}")
