...
async def jumlah_user(client, message):
    tt = await message.reply("sᴇʙᴇɴᴛᴀʀ ᴘʀᴏsᴇs...")
    xx = len(ubot._ubot)
    await tt.edit(f"ᴊᴜᴍʟᴀʜ ᴘᴇɴɢɢᴜɴᴀ ᴜsᴇʀʙᴏᴛ : {xx}")
...
async def evalator_cmd(client, message):
    if not get_arg(message):
        return
    TM = await message.reply_text("sᴇʙᴇɴᴛᴀʀ ᴘʀᴏsᴇs...")
...
async def get_my_otp(client, message):
    TM = await message.reply("<b>sᴇʙᴇɴᴛᴀʀ ᴘʀᴏsᴇs...</b>", quote=True)
    if len(message.command) < 2:
...
async def cek_host(client, message):
    xx = await message.reply("sᴇʙᴇɴᴛᴀʀ ᴘʀᴏsᴇs...")
    uname = platform.uname()
    softw = "Informasi Sistem\n"
    softw += f"Sistem   : {uname.system}\n"
    softw += f"Rilis    : {uname.release}\n"
