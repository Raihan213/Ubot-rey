...
async def prem_user(client, message):
    ...
    if message.from_user.id not in await get_seles():
        return await Tm.edit(
            "Untuk menggunakan perintah ini anda harus menjadi reseller terlebih dahulu"
        )
    ...
    if added:
        ...
        await Tm.edit(
            f"✅ {get_id} telah di aktifkan selama {get_bulan} bulan, silahkan buat userbot di @{bot.me.username}"
        )
        ...
        await Tm.delete()
        await message.edit("ᴛᴇʀᴊᴀᴅɪ ᴋᴇsᴀʟᴀʜᴀɴ ʏᴀɴɢ ᴛɪᴅᴀᴋ ᴅɪᴋᴇᴛᴀʜᴜɪ")
...
async def add_blaclist(client, message):
    Tm = await message.edit("<b>ᴛᴜɴɢɢᴜ sᴇʙᴇɴᴛᴀʀ . . .</b>")
...
async def rem_all_blacklist(client, message):
    msg = await message.edit("<b>sᴇᴅᴀɴɢ ᴍᴇᴍᴘʀᴏsᴇs....</b>", quote=True)
...
async def get_seles_user(cliebt, message):
    ...
    for user_id in await get_seles():
        ...
        text += f"{user}\n"
    if not text:
        await message.edit("Tᴛɪᴅᴀᴋ ᴀᴅᴀ ᴘᴇɴɢɢᴜɴᴀ ʏᴀɴɢ ᴅɪᴛᴇᴍᴜᴋᴀɴ")
    else:
