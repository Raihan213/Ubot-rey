...
<b><emoji id={emot_uptime}>🔘</emoji> ᴅᴀᴛᴇ:</b> <code>{str(uptime).replace('.', ',')} ms</code>
<b><emoji id={emot_mention}>✉️</emoji> ʀᴇʏ-ᴜsᴇʀʙᴏᴛ:</b> <a href=tg://user?id={client.me.id}>{client.me.first_name} {client.me.last_name or ''}</a>
"""
    else:
        _ping = f"""
<b>🏓 ᴘᴏɴɢ:</b> <code>{str(delta_ping).replace('.', ',')} ms</code>
<b>⏱ ᴅᴀᴛᴇ: <code>{uptime}</code></b>
<b>⚜ ʀᴇʏ-ᴜsᴇʀʙᴏᴛ:</b> <a href=tg://user?id={client.me.id}>{client.me.first_name} {client.me.last_name or ''}</a>
"""
